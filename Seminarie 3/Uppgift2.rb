...
##############################################################################
#
# This part defines the TDP007 language
#
##############################################################################

class TDP007
  def initialize
    @langParser = Parser.new('Logic') do
      @vars = {}

      token(/\s+/) # Remove whitespace
      token(/\w+/) { |m| m } # Extract words
      token(/./) { |m| m } # Extract everything left
      ...
      rule :assign do
        # Extracts the var and expression then saves it into hash.
        match('(', 'set', :var, :expr, ')') { |_, _, a, b, _| @vars[a.to_sym] = b }
      end
      ...
      rule :term do
        match('true') { |_a| true }
        match('false') { |_a| false }
        match('all') { |_a| @vars.each { |b| puts b } }
        match(:var)
      end

      rule :var do
        # Vars can only be big letters
        match(/[A-Z]/) { |a| !@vars[a.to_sym].nil? ? @vars[a.to_sym] : a }
      end
    end
  end
  ...
  def start
    puts 'WELCOME TO THE TDP007 LANGAUGE!'
    print '$ '
    str = gets
    if done(str)
      puts 'Bye...'
  ...
  def parse(statement)
    @langParser.parse statement
    end

  def log(state = false)
    @langParser.logger.level = if state
                                 Logger::DEBUG
                               else
                                 Logger::WARN
                               end
