...
class ArithmeticConstraint
  # Extended class to force operator switch.
  def inverse
...
class ConstraintParser < Parser
    ...
    @parser = super('constraints') do
      ...
      start :statement do
        match(:expr, '=', :term) do |lh, _, rh|
          ...
          @connectors.values.reject do |conn|
            ...
            conn.is_a?(ConstantConnector) || !(/^\w+$/ =~ conn.name)
          end
        end
      end
      ...
      rule :term do
        ...
        match(:term, '/', :atom) do |a, _, b|
          conn_a, conn_b, conn_c = get_connectors(a, '/', b)
          # a/b=c <=> b*c=a
          Multiplier.new(conn_a, conn_b, conn_c).inverse
        end
  ...
  def get_connectors(conn_a, op, conn_b)
    conn_a = conn_a.out unless conn_a.is_a? Connector

    conn_b = conn_b.out unless conn_b.is_a? Connector
  ...
  def replace_conn(lh, rh)
    lh_conn = get_connector(lh)
    rh_conn = get_connector(rh)
    conn = nil
    exp = nil
    ...
    else if lh.is_a?(ArithmeticConstraint)
           exp = lh
           conn = rh_conn
